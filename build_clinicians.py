...
# Roster tab uses full discipline names; the map's colour/legend system
# uses short codes. Keep this mapping here rather than in the map, since
# this is the one place that should ever need to change it.
DISCIPLINE_CODE = {
    "physio": "PT",
    "ot": "OT",
    "speech": "Speech",
...
def norm_name(s: str) -> str:
    # Strips accents/curly-vs-straight apostrophes etc, so small
    # formatting differences between the two tabs don't break the join.
    # This is a safety net, NOT a substitute for the Roster tab using
    # Cliniko's exact practitioner name — that match happens upstream,
    # inside the Cliniko feed itself.
...
def load_roster(service):
    ...
    for row in rows:
        ...
        if status_norm not in ("active", "leave"):
            continue  # "inactive"/departed/blank — not shown on the map
